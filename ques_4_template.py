...
def maxpool(h):
    m = np.zeros((10816,np.shape(h)[1]))
    for l in range(np.shape(h)[1]):
        for n in range(64):
            for i in range(13):
                for j in range(13):
                    k = 2*j + i*52 + n*676
                    m[j+13*i+n*169,l] = np.max([h[k,l],h[k+1,l],h[k+26,l],h[k+27,l]])
                    #if n<2:
                        #print(m[j+13*i+n*169])
...
def forward_prop (x, y, Ws,bs):
    ...
    for l in range(0,len(Ws)-1):
        print(l)
        print("mulitplying")

        Zl = Ws[l]@H_l_1 + np.atleast_2d(bs[l]).T
        ...
        if(l==0):
            Hl = Hl.reshape(64,13,13)
            Hl = np.moveaxis(Hl,0,-1)
            Hl = Hl.flatten()
            Hl = np.atleast_2d(Hl).T
        #print(Hl[0:13])
        hs.append(Hl)
        zs.append(Zl)
        H_l_1 = Hl
    ...
    Zl = Ws[2]@H_l_1 + np.atleast_2d(bs[2]).T
    #print(Zl[-1])
...
#                                    weightsAndBiases))
    ...
    X_te = np.atleast_2d(X_te[:,1]/255).T
    yte = (np.load("fashion_mnist_test_labels.npy"))

    #Perform one hot encoding
    yte = one_hotencoding(yte)

    yte = np.atleast_2d(yte[:,1]).T


    #Find error on test set with optimal hyperparameter set and corresponding weights

    #weightsAndBiases_conv = []
    #with open("file.txt", "r") as f:
    #    for line in f:
    #        weightsAndBiases_conv.append(int(line.strip()))

    with open("test", "rb") as fp:   # Unpickling
        weightsAndBiases_conv = pickle.load(fp)


    Ws = []

    Ws.append(weightsAndBiases_conv[0][0])
    Ws.append(weightsAndBiases_conv[0][1].T)
    Ws.append(weightsAndBiases_conv[0][2].T)

    bs = []

    bs.append(weightsAndBiases_conv[1][0])
    bs.append(weightsAndBiases_conv[1][1])
    bs.append(weightsAndBiases_conv[1][2])

    print(yte)

    test_set_cost,_,_,yhat = forward_prop(X_te, yte, Ws,bs)
    count = 0
